import React from 'react';

export type CompanyName = 'Google' | 'Intuit';

interface CompanyWordmarkProps {
  name: CompanyName;
  className?: string;
}

function GoogleWordmark({ className }: { className?: string }) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 74 24"
      className={className}
      role="img"
      aria-label="Google"
    >
      <title>Google</title>
      <path
        fill="currentColor"
        d="M9.24 8.19v2.46h5.88c-.18 1.38-.64 2.39-1.34 3.1-.86.86-2.2 1.8-4.54 1.8-3.62 0-6.45-2.92-6.45-6.54s2.83-6.54 6.45-6.54c1.95 0 3.38.77 4.43 1.76L15.4 2.5C13.94 1.08 11.98 0 9.24 0 4.28 0 .11 4.04.11 9s4.17 9 9.13 9c2.68 0 4.7-.88 6.28-2.52 1.62-1.62 2.13-3.91 2.13-5.75 0-.57-.04-1.1-.13-1.54H9.24z"
      />
      <path
        fill="currentColor"
        d="M25 6.19c-3.21 0-5.83 2.44-5.83 5.81 0 3.34 2.62 5.81 5.83 5.81s5.83-2.46 5.83-5.81c0-3.37-2.62-5.81-5.83-5.81zm0 9.33c-1.76 0-3.28-1.45-3.28-3.52 0-2.09 1.52-3.52 3.28-3.52s3.28 1.43 3.28 3.52c0 2.07-1.52 3.52-3.28 3.52z"
      />
      <path
        fill="currentColor"
        d="M53.58 7.49h-.09c-.57-.68-1.67-1.3-3.06-1.3C47.53 6.19 45 8.72 45 12c0 3.26 2.53 5.81 5.43 5.81 1.39 0 2.49-.62 3.06-1.32h.09v.81c0 2.22-1.19 3.41-3.1 3.41-1.56 0-2.53-1.12-2.93-2.07l-2.22.92c.64 1.54 2.33 3.43 5.15 3.43 2.99 0 5.52-1.76 5.52-6.05V6.49h-2.42v1zm-2.93 8.03c-1.76 0-3.1-1.5-3.1-3.52 0-2.05 1.34-3.52 3.1-3.52 1.74 0 3.1 1.5 3.1 3.54.01 2.03-1.36 3.5-3.1 3.5z"
      />
      <path
        fill="currentColor"
        d="M38 6.19c-3.21 0-5.83 2.44-5.83 5.81 0 3.34 2.62 5.81 5.83 5.81s5.83-2.46 5.83-5.81c0-3.37-2.62-5.81-5.83-5.81zm0 9.33c-1.76 0-3.28-1.45-3.28-3.52 0-2.09 1.52-3.52 3.28-3.52s3.28 1.43 3.28 3.52c0 2.07-1.52 3.52-3.28 3.52z"
      />
      <path fill="currentColor" d="M58 .24h2.51v17.57H58z" />
      <path
        fill="currentColor"
        d="M68.26 15.52c-1.3 0-2.22-.59-2.82-1.76l7.77-3.21-.26-.66c-.48-1.3-1.96-3.7-4.97-3.7-2.99 0-5.48 2.35-5.48 5.81 0 3.26 2.46 5.81 5.76 5.81 2.66 0 4.2-1.63 4.84-2.57l-1.98-1.32c-.66.96-1.56 1.6-2.86 1.6zm-.18-7.15c1.03 0 1.91.53 2.2 1.28l-5.25 2.17c0-2.44 1.73-3.45 3.05-3.45z"
      />
    </svg>
  );
}

function IntuitWordmark({ className }: { className?: string }) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 845.85 170.09"
      className={className}
      role="img"
      aria-label="Intuit"
    >
      <title>Intuit</title>
      <path
        fill="currentColor"
        d="m434.23 98.424c0 41.39 34.318 71.666 77.917 71.666 43.596 0 77.953-30.276 77.953-71.666v-95.093h-42.216v90.191c0 22.342-15.493 37.386-35.851 37.386-20.358 0-35.851-14.969-35.851-37.386v-90.191h-42.213zm268.66-57.034h50.37v125.56h42.213v-125.56h50.373v-38.134h-142.96zm-27.282-38.134h-42.213v163.69h42.213zm-411.92 38.134h50.372v125.56h42.214v-125.56h50.334v-38.134h-142.92zm-221.47-38.134h-42.214v163.69h42.214zm199.24 68.373c0-41.391-34.318-71.629-77.953-71.629-43.636 0-77.953 30.238-77.953 71.629v95.317h42.214v-90.191c0-22.342 15.494-37.386 35.852-37.386 20.358 0 35.852 14.969 35.852 37.386v90.191h42.214l-0.26191-95.317z"
      />
    </svg>
  );
}

export function CompanyWordmark({ name, className }: CompanyWordmarkProps) {
  if (name === 'Google') {
    return <GoogleWordmark className={className} />;
  }
  return <IntuitWordmark className={className} />;
}

export function hasCompanyWordmark(label: string): label is CompanyName {
  return label === 'Google' || label === 'Intuit';
}
